...
var _numGetServiceAttempts = 30;
var _getServiceAttemptTimeout = 2000;
var _numGetServieAttemptsBeforeScan = 5;
...
WiFi.prototype.restart = function(callback) {
  debug('restart');
  var self = this;
...
WiFi.prototype.enable = function(callback) {
  debug('enable');
  if (this.getProperty('Powered') !== true){
    debug('enabling')
    this.setProperty('Powered', true, function(err) {
      if (err && err.message.match(/AlreadyEnabled/)){
        debug('wifi already enabled');
        err = undefined;
      }
...
WiFi.prototype.disable = function(callback) {
  debug('Disable');
  if (this.getProperty('Powered') !== false){
    debug('Disabling');
    this.setProperty('Powered', false, function(err) {
      if (err && err.message.match(/AlreadyDisabled/)){
        debug('wifi already disabled');
        err = undefined;
      }
      if (err) {
        debug('[Error] disabling wifi failed: ', err);
      }
      callback();
    });
  } else {
...
WiFi.prototype.openHotspot = function(ssid, passphrase, callback) {
  ...
  async.series([
    ...
    function doOpen(next) {
      ...
      self.iface.enableTethering(ssid, passphrase, next);
    }
  ],
  function(err) {
    if (err) {
...
function searchService(query, numRetries, callback) {
  ...
  async.retry(numRetries, function(nextRetry) {
    ...
    if (attempt > 5 && !scanning){
      debug('Doing a scan');
      self.scan(); // start scan, but don't wait for it
      scanning = true;
    } else {
      debug('not scanning this time');
    }
